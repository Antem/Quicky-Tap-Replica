...
#import "RelentlessMedium.h"

@interface RelentlessMedium ()

@end

@implementation RelentlessMedium
...
-(void)reportScore
{
    GKScore *score = [[GKScore alloc] initWithLeaderboardIdentifier:@"Relentless_Medium_Leaderboard"];
    int64_t GameCenterScoreTwenty = HighScoreNumber;
    score.value = GameCenterScoreTwenty;
...
-(void)EndGame
{
    ...
    if (Score > HighScoreNumber) {
        ...
        [[NSUserDefaults standardUserDefaults] setInteger:HighScoreNumber forKey:@"RelentlessMediumHighScoreSaved"];
    }
    [self reportScore];
...
- (void)viewDidLoad {
    ...
    HighScoreNumber = [[NSUserDefaults standardUserDefaults] integerForKey:@"RelentlessMediumHighScoreSaved"];
    
    if (HighScoreNumber == 0) {
        NSString *Relent = @"You are given 30 seconds, to tap the big button as many times as you can before the timer runs out.";
