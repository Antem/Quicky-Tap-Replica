...
#import "Mayhem Easy.h"
#import <AudioToolbox/AudioToolbox.h>
#import <CoreFoundation/CoreFoundation.h>
#import <GameKit/GameKit.h>

BOOL GameOver;
float ScoreNumber, ButtonAmount, TheLastNumber;
double HighScoreNumber;

@interface Mayhem_Easy ()
{
    IBOutlet UIImageView *OtherButtonTwelve;
    IBOutlet UIImageView *OtherButtonThirteen;
    IBOutlet UIImageView *OtherButtonFourteen;
    IBOutlet UIImageView *OtherButtonFithteen;
...
-(void)reportScore
{
    GKScore *score = [[GKScore alloc] initWithLeaderboardIdentifier:@"Mayhem_Easy_Leaderboard"];
    int64_t GameCenterScoreTen = (int64_t) (HighScoreNumber * 100.00f);
    score.value = GameCenterScoreTen;
...
-(void)EndGame
{
    ...
    TheLastNumber = ScoreNumber;
    
    if (TheLastNumber < HighScoreNumber || HighScoreNumber == 0) {
        HighScoreNumber = TheLastNumber;
        [[NSUserDefaults standardUserDefaults] setDouble:HighScoreNumber forKey:@"HighScoreMayhemEasySaved"];
    }
    [self reportScore];
    LastScore.text = [NSString stringWithFormat:@"%.2f", TheLastNumber];
    HighScoreLbl.text = [NSString stringWithFormat:@"%.2f", HighScoreNumber];
...
- (void)viewDidLoad
{
    ...
    HighScoreNumber = [[NSUserDefaults standardUserDefaults] doubleForKey:@"HighScoreMayhemEasySaved"];
