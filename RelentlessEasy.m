...
#import "RelentlessEasy.h"

@interface RelentlessEasy ()

@end

@implementation RelentlessEasy
...
-(void)reportScore
{
    GKScore *score = [[GKScore alloc] initWithLeaderboardIdentifier:@"Relentless_Easy_Leaderboard"];
    int64_t GameCenterScoreNineTeen = HighScoreNumber;
    score.value = GameCenterScoreNineTeen;
...
-(void)EndGame
{
    ...
    if (Score > HighScoreNumber) {
        ...
        [[NSUserDefaults standardUserDefaults] setInteger:HighScoreNumber forKey:@"RelentlessEasyHighScoreSaved"];
    }
    [self reportScore];
...
-(void)viewDidLoad {
    ...
    HighScoreNumber = [[NSUserDefaults standardUserDefaults] integerForKey:@"RelentlessEasyHighScoreSaved"];
    
    if (HighScoreNumber == 0) {
            NSString *Relent = @"You are given 15 seconds, to tap the big button as many times as you can before the timer runs out.";
