...
-(void)reportScore
{
    GKScore *score = [[GKScore alloc] initWithLeaderboardIdentifier:@"RevClassic_Leaderboard"];
    int64_t GameCenterScoreThirteen = (int64_t) (HighScoreNumber * 100.00f);
    score.value = GameCenterScoreThirteen;
...
-(void)GameOver
{
    ...
    if (Counter < HighScoreNumber || HighScoreNumber == 0) {
        ...
        [[NSUserDefaults standardUserDefaults] setDouble:HighScoreNumber forKey:@"HighScoreRevClassicSaved"];
    }
    [self reportScore];
...
- (void)viewDidLoad
{
    ...
    HighScoreNumber = [[NSUserDefaults standardUserDefaults] doubleForKey:@"HighScoreRevClassicSaved"];
    
    if (HighScoreNumber == 0) {
        NSString *Rev = @"You have to tap the black this time. If you tap your selected button, the game will be over. This game mode is exactly the same as classic medium, but reverse!!";
