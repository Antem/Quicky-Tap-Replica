...
@interface Mayhem_Medium ()
{
    IBOutlet UIButton *Home, *Replay;
    IBOutlet UIImageView *Display, *LostDisplay;
    IBOutlet UIImageView  *OtherButtonThirteen, *OtherButtonFourteen, *OtherButtonFithteen;
...
-(void)reportScore
{
    GKScore *score = [[GKScore alloc] initWithLeaderboardIdentifier:@"Mayhem_Medium_Leaderboard"];
    int64_t GameCenterScoreEleven = (int64_t) (HighScoreNumber * 100.00f);
    score.value = GameCenterScoreEleven;
...
-(void)EndGame
{
    ...
    if (ScoreNumber < HighScoreNumber || HighScoreNumber == 0) {
        ...
        [[NSUserDefaults standardUserDefaults] setDouble:HighScoreNumber forKey:@"HighScoreMayhemMediumSaved"];
    }
    [self reportScore];
...
- (void)viewDidLoad
{
    ...
    HighScoreNumber = [[NSUserDefaults standardUserDefaults] doubleForKey:@"HighScoreMayhemMediumSaved"];
