...
-(void)reportScore
{
    GKScore *score = [[GKScore alloc] initWithLeaderboardIdentifier:@"RevZen_Leaderboard"];
    int64_t GameCenterScoreFourteen = ButtonPressed;
    score.value = GameCenterScoreFourteen;
...
-(void)EndGame
{
    ...
    if (ButtonPressed > HighScoreNum) {
        ...
        [[NSUserDefaults standardUserDefaults] setInteger:HighScoreNum forKey:@"HighScoreRevZenSaved"];
    }
    [self reportScore];
...
- (void)viewDidLoad
{
    ...
    HighScoreNum = [[NSUserDefaults standardUserDefaults] integerForKey:@"HighScoreRevZenSaved"];
    
    if (HighScoreNum == 0) {
        NSString *Rev = @"You have to tap the black this time. If you tap your selected button, the game will be over. This game mode is exactly the same as zen medium, but reverse!!";
