...
-(void)reportScore
{
    GKScore *score = [[GKScore alloc] initWithLeaderboardIdentifier:@"Classic_Hard_Leaderboard"]; //Leaderboard name
    int64_t GameCenterScoreThree = (int64_t) (HighScoreNumber * 100.00f); // All numbers must be sent in a int64, if you have got a decimal point you do what i have doe here, highscore is a float/double *100 to gte .2dp *1000 for .3dp
    score.value = GameCenterScoreThree;
...
-(void)GameOver
{
    ...
    if (Counter < HighScoreNumber || HighScoreNumber == 0) {
        ...
        [[NSUserDefaults standardUserDefaults] setDouble:HighScoreNumber forKey:@"HighScoreHardSaved"];
    }
    [self reportScore];
...
- (void)viewDidLoad
{
    ...
    HighScoreNumber = [[NSUserDefaults standardUserDefaults] doubleForKey:@"HighScoreHardSaved"];
    
    if (HighScoreNumber == 0) {
        NSString *Class = @"You have 60 buttons to tap as fast as you can! And don't hit the black buttons or the game will be over.";
