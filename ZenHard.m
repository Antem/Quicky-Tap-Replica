...
-(void)reportScore
{
    GKScore *score = [[GKScore alloc] initWithLeaderboardIdentifier:@"Zen_Hard_Leaderboard"];
    int64_t GameCenterScoreSix = HighScoreNum;
    score.value = GameCenterScoreSix;
...
-(void)EndGame
{
    ...
    if (ButtonPressed > HighScoreNum) {
        ...
        [[NSUserDefaults standardUserDefaults] setInteger:HighScoreNum forKey:@"HighScoreZenHardSaved"];
    }
    [self reportScore];
...
- (void)viewDidLoad
{
    ...
    HighScoreNum = [[NSUserDefaults standardUserDefaults] integerForKey:@"HighScoreZenHardSaved"];
    
    if (HighScoreNum == 0) {
        NSString *Zen = @"You have a 60 seconds to tap as much buttons as you can. If you tap a black button before the timer runs out, you keep the same score however the game is over.";
